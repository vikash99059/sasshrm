...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const baseStyles = 'group inline-flex items-center justify-center font-semibold rounded-xl transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed select-none active:scale-[0.97] btn-tactile cursor-pointer';
    ...
    const variants = {
      ...
      ghost: 'hover:bg-slate-100/80 text-slate-600 hover:text-slate-900 dark:hover:bg-slate-800 dark:text-slate-400 dark:hover:text-slate-100 focus:ring-slate-500 active:scale-[0.97]',
    ...
    return (
      ...
      >
        {isLoading ? (
          ...
          leftIcon && <span className="transition-transform duration-200 group-hover:scale-105 flex items-center">{leftIcon}</span>
        )}
        {children}
        {!isLoading && rightIcon && (
